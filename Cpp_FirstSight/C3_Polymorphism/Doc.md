...
0x7ffffe956de0  test2 : Class A 100

0x64  test2 : Class A 0

如果输出的时候， 我们输出一下地址，就会发现指针变成了100(0x64就是100)

> 实际上类的方法调用的时候有一个隐藏的类的指针参数，位于开头

```cpp
void test(A *this, int x)
```

这也就回答了为什么成员方法才能调用this指针，类方法没有这个隐藏属性


## 访问者模式

实现接口，用接口调用this指针解析出类型
